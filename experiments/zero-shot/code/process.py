...
def extract_meta_data(path):
    ...
    with gzip.open(path) as f:
        for line in tqdm(f):
            ...
            attr_dict = dict()
            asin = line['asin']
            category = ' ' if 'category' not in line else ' '.join(line['category'])
            brand = ' ' if 'brand' not in line else line['brand']
            title = ' ' if 'title' not in line else line['title']
...
for line in tqdm(gin):
    ...
    if 'reviewerID' in line and 'asin' in line and 'unixReviewTime' in line:
